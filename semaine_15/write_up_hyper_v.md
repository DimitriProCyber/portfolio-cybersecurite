...
La première étape est de vérifier la configuration réseau de PC02 grâce à la commande `ipconfig /all`. J'ai pu constater dans les résultats, que l'IP de la VM était 169.254.58.167. Les adresses IP au format 169.254.0.0/16 sont des adresses APIPA (Automatic Private IP Addressing), et sont attribuées à une machine lorsque les requêtes DHCP de celle-ci échouent. Il s'agit d'un réseau privé, qui n'est pas routable sur Internet et la machine ne pourra communiquer qu'avec des cartes réseau configurées en APIPA également. Ici, DC01 n'étant pas encore allumé il était normal que PC02 s'est vu attribuer une adresse APIPA, car le serveur DHCP n'était pas joignable.
...
Lorsque j'ai découvert VirtualBox, celui-ci m'a semblé plus simple à appréhender au premier abord qu'Hyper-V. En effet, VirtualBox est plus intuitif et plus direct : on installe le logiciel, on télécharge l'ISO que l'on souhaite utiliser, on règle quelques paramètres et on peut se lancer. Pour Hyper-V, il est avant tout nécessaire de comprendre l'architecture de la virtualisation, qui demande de connaitre les différents types de switchs ainsi que la distinction entre partition parente et enfants par exemple. Sans ça, il n'est pas possible de paramétrer correctement la VM et de l'adapter au mieux à notre besoin. Cette différence reflète directement l'opposition entre un hyperviseur de Type 2, applicatif et immédiat, et un hyperviseur de Type 1, qui impose de comprendre son architecture avant toute utilisation.

Lors du premier démarrage de la VM installée grâce à Hyper-V, la version de l'ISO Windows étant assez récente, le hash de l'image signée n'était pas autorisé dans la base de certificats, ce qui m'a poussé à devoir désactiver le Secure Boot afin de démarrer correctement. Cela est dû au fait qu'Hyper-V applique les mêmes exigences de sécurité qu'un PC classique imposerait pour installer Windows 11, permettant de reproduire les exigences en matière de sécurité qu'impose Windows pour utiliser son OS. A contrario, VirtualBox n'a pas les mêmes contraintes, ce qui permet d'utiliser plus facilement l'OS, mais qui est donc moins représentatif de ce qui peut se passer sur un vrai poste de travail. VirtualBox est donc plus permissif et permet de simuler rapidement un environnement éducatif, alors qu'Hyper-V est plus fidèle aux environnements physiques et permet de les simuler pour les utiliser dans des contextes plus professionnels car les exigences de sécurité sont respectées.

Enfin, Hyper-V étant un hyperviseur de Type 1, il permet d'exécuter les différentes VM sans passer par des couches applicatives supplémentaires, contrairement à VirtualBox (hyperviseur de Type 2), où les différentes VMs s'exécutent sur l'OS principal de la machine. Le résultat est que les VM qui tournent grâce à Hyper-V sont plus rapides et peuvent utiliser le matériel à leur plein potentiel. La possibilité d'allouer la RAM de manière dynamique, permet de renforcer cette réactivité en l'ajustant automatiquement en fonction des besoins réels. De plus, Hyper-V permet une isolation plus stricte des différentes VM, en offrant la possibilité de cloisonner la partition parente au même titre que les partitions enfants. Sur un hyperviseur de Type 2, les VMs s'exécutant sur l'OS principal, celui-ci aura toujours accès à toutes les VM. En cas de compromission de l'OS principal, toutes les VM deviennent automatiquement vulnérables, ce qui ne sera pas le cas sur un hyperviseur de Type 1 correctement configuré.
...
L'adressage IP de PC02, actuellement en DHCP dynamique bien que stable, gagnerait à être fixé par une réservation DHCP basée sur l'adresse MAC, à l'image de ce qui a déjà été mis en place pour le serveur Debian DMZ, ce qui garantirait la stabilité nécessaire à des règles de pare-feu ou une supervision réseau fiables.
...
Ce lab m'a permis de découvrir un nouveau type d'hyperviseur, utilisé fréquemment en environnement professionnel. J'ai pu appréhender la différence entre hyperviseur de Type 1 et de Type 2, ainsi que les avantages de chaque type en fonction de l'utilisation que l'on souhaite en faire. J'ai pu découvrir la théorie sur les hyperviseurs de Type 1 (partition parente/enfant, type de switch...) et mettre directement en pratique afin de rendre cela tangible. Cette pratique m'a aussi permis d'avoir une première expérience sur l'utilisation d'une telle technologie, qui ne sera pas négligeable en environnement professionnel. 

Les problèmes que j'ai pu rencontrer durant le lab, n'étaient pas attendus mais se sont avérés utiles pour approfondir mes connaissances, notamment en termes de dépannage. J'ai eu l'occasion d'apprendre à isoler les causes potentielles me permettant de mieux comprendre l'importance de chaque étape, et la manière dont elles étaient connectées entre elles. 

Je pense que ce lab, même s'il ne fait pas de moi un expert, me permet d'avoir une première expérience valorisante sur un métier de technicien réseau ou d'administrateur systèmes et réseaux junior. J'ai en effet pu découvrir comment créer des machines virtuelles, ce qui est une opération courante en entreprise, faire face à des problèmes tant au niveau de l'ISO utilisé qu'au moment de la jonction, ce qui peut arriver également en entreprise. Grâce à ce lab, j'ai maintenant des pistes me permettant d'appliquer un premier niveau d'autonomie en entreprise, et me permettant d'approfondir mes connaissances par la suite.
