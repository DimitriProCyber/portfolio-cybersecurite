...
8. [Recommandations pour une PME](#8-recommandations-pour-une-pme)
...
**Ping vers la passerelle** (PC-RH-1 vers 192.168.10.1) : succès, TTL=255. Ici c'est le routeur qui répond et donc qui défini le TTL. La valeur initiale du TTL d'un équipement Cisco est de 255 et le paquet arrivant à destination sans passer par un autre équipement de couche 3, il reste à 255. Le résultat valide le lien trunk entre SW-RH et R-NL ainsi que la sous-interface Gig0/0.10.
...
En production, cet échange se ferait via un canal sécurisé : email chiffré ou gestionnaire de secrets.

#### Fichiers de configuration

**Sur Kali (/etc/wireguard/wg0.conf) :**
...
**Sur Debian (/etc/wireguard/wg0.conf) :**
...
- `AllowedIPs = 10.0.0.2/32` : filtre de sécurité — seul le trafic de/vers cette IP exacte (/32 = une seule adresse) est accepté via ce pair. À distinguer du Address /24 qui définit le réseau local
...
`wg-quick up wg0` exécute automatiquement : création de l'interface réseau wg0, chargement de la configuration, assignation de l'IP tunnel, activation de l'interface avec MTU 1420 (réduit depuis 1500 pour compenser les en-têtes WireGuard d'environ 60-80 octets et éviter la fragmentation des paquets).
...
- **Handshake Initiation** (Kali → Debian) : Wireshark identifie le protocole WireGuard et affiche le type de message. C'est la phase Curve25519 — les deux machines négocient et dérivent la clé de session commune sans jamais la faire transiter sur le réseau.
- **Handshake Response** (Debian → Kali) : confirmation que le tunnel est établi.

**Phase 2 — Transport des données**

Les paquets suivants apparaissent avec le type "Transport Data" et une indication de taille (datalen=96). Le panneau hexadécimal en bas de Wireshark affiche des octets aléatoires — il est impossible d'y lire quoi que ce soit. Wireshark voit l'enveloppe UDP mais ne peut pas déchiffrer le contenu, chiffré par ChaCha20.
...
### 4.5 Problèmes rencontrés et résolus
...
| Fichier `wgo.conf` créé au lieu de `wg0.conf` | Confusion entre la lettre O et le chiffre 0 | `mv /etc/wireguard/wgo.conf /etc/wireguard/wg0.conf` |
| Wireshark "non autorisé à capturer" | Bit exécutable manquant sur dumpcap pour le groupe wireshark | `chmod +x /usr/bin/dumpcap` + `setcap cap_net_raw,cap_net_admin=eip /usr/bin/dumpcap` |
...
Le NAT (Network Address Translation) est la solution : le routeur remplace l'IP privée du poste par l'IP publique de l'entreprise avant d'envoyer le paquet sur internet. Quand la réponse revient, le routeur retrouve à quel poste interne elle est destinée et retraduit.

Le **PAT** (Port Address Translation), aussi appelé NAT overload, est la variante utilisée dans 99% des PME : une seule IP publique est partagée entre tous les postes internes. La distinction se fait grâce aux numéros de port — chaque connexion reçoit un port source unique, ce qui permet au routeur de savoir à qui retourner la réponse.

**Niveau 2 — Technique**

Le PAT réécrit deux champs dans chaque paquet : l'IP source (privée → publique) et le port source (remplacé par un port unique assigné par le routeur). Le routeur maintient une **table NAT** qui associe chaque paire (IP privée : port source) à un port public.
...
**Trois variantes NAT à connaître :**

- **NAT statique** : 1 IP privée ↔ 1 IP publique fixe permanente. Utilisé pour les serveurs exposés (web, mail). Syntaxe : `ip nat inside source static [IP privée] [IP publique]`
- **NAT dynamique** : pool d'IPs publiques, attribution temporaire à la demande. Rare en PME.
- **PAT (NAT overload)** : N IPs privées partagent 1 IP publique via les ports. Cas standard PME. Syntaxe : `ip nat inside source list [ACL] interface [outside] overload`

**ACL standard vs étendue :**
- ACL standard (numéros 1-99) : filtre uniquement sur l'IP source
- ACL étendue (numéros 100-199) : filtre sur source, destination, port et protocole

**Wildcard mask :** utilisé dans les ACL Cisco pour définir la plage d'IPs concernées. Calcul : 255.255.255.255 − masque normal. Pour un réseau /24 : 255.255.255.255 − 255.255.255.0 = 0.0.0.255. Le 0 signifie "ce bit doit correspondre exactement", le 255 signifie "ce bit peut être n'importe quoi".
...
Server0 (serveur web externe simulé) est connecté à Gig0/2 avec l'IP 203.0.113.2 et la passerelle 203.0.113.1. La passerelle 203.0.113.1 est nécessaire pour que le serveur puisse répondre aux paquets dont l'IP destination a été traduite par R-NL.
...
L'ACL 1 (standard) liste les trois sous-réseaux internes autorisés à être traduits. Le wildcard 0.0.0.255 couvre les 256 adresses de chaque /24.
...
- `interface GigabitEthernet0/2` : l'IP publique utilisée pour la traduction est celle de cette interface (203.0.113.1)
...
Après un ping depuis PC-RH-1 vers 203.0.113.2, cette commande affiche les entrées de la table NAT avec les colonnes Inside local / Inside global / Outside local / Outside global. Les entrées sont dynamiques et disparaissent après quelques secondes d'inactivité — comportement normal.

### 5.3 Théorie : IDS et IPS

**Niveau 1 — Vision PME**

Un IDS (Intrusion Detection System) est un système passif : il observe le trafic réseau, détecte les comportements suspects, et envoie des alertes. Il ne bloque rien. Un IPS (Intrusion Prevention System) est actif : il détecte et bloque en temps réel.

L'analogie : l'IDS est un vigile qui appelle le responsable quand il voit quelqu'un de suspect. L'IPS est un vigile qui bloque physiquement l'intrus avant qu'il entre.

**Niveau 2 — Technique**

Deux modes de détection :

- **Par signature** : comparaison du trafic avec une base de signatures d'attaques connues. Efficace mais aveugle aux attaques inconnues (zero-day).
- **Par anomalie** : comparaison avec une baseline de trafic normal. Détecte les comportements inhabituels même sans signature connue, mais génère davantage de faux positifs.

Positionnement dans l'architecture :

- **IDS** : connecté en coupure passive (port miroir du switch) — reçoit une copie du trafic sans être dans le chemin. Ne peut pas bloquer.
- **IPS** : placé en coupure active dans le flux — tout le trafic passe par lui. Peut bloquer en temps réel.

Dans l'architecture NordLogistique, un IPS se placerait entre R-NL et les switches d'accès — après le routeur, avant le réseau interne. Le trafic a déjà été filtré une première fois par les ACL du routeur. L'IPS analyse ce qui a passé ce premier filtre et bloque ce qui reste suspect avant que ça atteigne les postes des salariés.

**Pourquoi déployer un IDS avant un IPS ?** Un IPS mal configuré peut bloquer du trafic légitime et paralyser la production. La pratique courante consiste à déployer en mode IDS d'abord, observer le trafic pendant quelques semaines pour affiner les règles, puis basculer en IPS une fois la baseline établie.

**Exemples d'outils :** Suricata (open source, IDS/IPS — déjà croisé dans les labs Splunk BOTS v1), Snort (open source, référence historique).

### 5.4 Diagnostic de pannes réseau

#### Méthodologie

Le diagnostic réseau professionnel suit toujours deux règles non négociables, quelle que soit l'intuition initiale :
...
PC inspecté en premier — IP, masque, passerelle corrects. Carte réseau active. Le problème vient d'un équipement en amont.

SW-RH inspecté avec `show ip interface brief` : Fa0/1 et Fa0/2 up/up (ports PCs RH — normaux), Fa0/24 up/up (trunk inter-switches — normal), **Gig0/1 administratively down/down** (trunk vers R-NL — anormal).

Confirmation avec `show interfaces Gig0/1 switchport` : Administrative Mode trunk, Operational Mode **down**.

*Cause identifiée :* Gig0/1 de SW-RH est en shutdown — le lien entre SW-RH et R-NL est coupé. Couche L1.
...
*Validation :* ping PC-RH-1 → 192.168.10.1 — succès.
...
Le ping intra-VLAN fonctionne : L1 et L2 côté PC sont corrects. La communication intra-VLAN ne passe pas par la passerelle — les machines du même réseau se joignent directement via le switch au niveau L2. Le problème est donc L3, côté PC.

PC-INFO-1 inspecté (Desktop → IP Configuration) : IP 192.168.20.10, masque 255.255.255.0, **passerelle 192.168.20.254** au lieu de 192.168.20.1.

*Explication du symptôme :* quand PC-INFO-1 veut joindre une machine hors de son réseau, il envoie le paquet à sa passerelle. Mais 192.168.20.254 n'existe pas — personne ne répond à cette adresse. En revanche, pour joindre PC-INFO-2 (même sous-réseau), la passerelle n'est pas consultée — la communication fonctionne.

*Cause identifiée :* mauvaise passerelle saisie manuellement. En production, ce type d'erreur survient lors d'une faute de frappe ou si le serveur DHCP distribue une mauvaise passerelle par défaut. Couche L3.

*Correction :* Desktop → IP Configuration → Default Gateway → 192.168.20.1.

*Validation :* ping PC-INFO-1 → PC-RH-1 — succès.
...
Deux PCs touchés simultanément : le problème vient d'un équipement en amont. SW-INFO-DIR inspecté.

`show ip interface brief` : Gig0/1 up/up — L1 correct.
...
Fa0/24   10,20,30
Gig0/1   10,20          ← VLAN 30 absent
```

*Cause identifiée :* VLAN 30 absent des VLANs autorisés sur le trunk Gig0/1 de SW-INFO-DIR. Les trames VLAN 30 ne peuvent pas passer vers le routeur. Couche L2.

*Correction :*

```
SW-INFO-DIR(config)# interface GigabitEthernet0/1
SW-INFO-DIR(config-if)# switchport trunk allowed vlan add 30
```

`switchport trunk allowed vlan add 30` ajoute le VLAN 30 à la liste existante sans écraser les autres VLANs. Sans le mot-clé `add`, la commande remplacerait toute la liste par "30 uniquement" — les VLANs 10 et 20 disparaîtraient. Erreur classique en production.

*Validation :* `show interfaces trunk` — Gig0/1 affiche 10,20,30. Ping PC-DIR-1 → PC-RH-1 — succès.
...
Tous les PCs touchés simultanément : problème sur le routeur. R-NL inspecté couche par couche.

L1 — `show ip interface brief` : toutes les interfaces up/up. Correct.

L2 — Sous-interfaces opérationnelles. Correct.

L3 — `show ip route` : entrée "C" (Connected) pour 203.0.113.0/24 via Gig0/2. R-NL sait où se trouve la destination. Table de routage correcte.

L3/NAT — `show running-config` : interfaces inside/outside correctement déclarées, ACL présentes. En revanche, **la ligne `ip nat inside source list 1 interface GigabitEthernet0/2 overload` est absente**.

*Cause identifiée :* sans cette règle, le routeur ne traduit pas les adresses — les paquets partent avec leur IP privée source (192.168.x.x), non routable sur internet. Couche L3/NAT.

*Correction :*

```
R-NL(config)# ip nat inside source list 1 interface GigabitEthernet0/2 overload
```

*Validation :* ping PC-RH-1 → 203.0.113.2 — succès (premier paquet perdu pendant l'initialisation de la table NAT — comportement normal sur Packet Tracer).
...
*[À rédiger par Dimitri — prends du recul sur l'ensemble des trois phases. Réponds à ces questions : qu'est-ce que cette infrastructure apporte concrètement à NordLogistique par rapport à la situation de départ ? Quels risques ont été réduits et comment ? En quoi les trois phases sont-elles complémentaires et forment-elles un tout cohérent ? Environ 150 mots.]*
...
2. **`no ip domain-lookup`** — évite les blocages de 30 secondes sur fautes de frappe en CLI
3. **`enable secret` plutôt que `enable password`** — hashage MD5 vs stockage en clair
4. **Bannière MOTD** — avertissement légal obligatoire avant toute action judiciaire
5. **Native VLAN changé vers VLAN inutilisé (999)** — protection contre le VLAN hopping
6. **Ports inutilisés désactivés ET dans VLAN poubelle** — défense en profondeur (isolation logique + physique)
...
9. **`copy running-config startup-config` après chaque bloc** — persistance au redémarrage
...
**ACL inter-VLAN absentes.** Le routeur laisse passer tout le trafic inter-VLAN sans restriction. En production, des ACL étendues contrôleraient finement les flux — par exemple, le département RH ne devrait pas pouvoir accéder aux serveurs de l'Informatique sur tous les ports.

**Authentification Cisco basique.** Les mots de passe utilisés (`enable secret` type 5 — MD5) sont acceptables en lab mais insuffisants en production. La recommandation actuelle est le type 9 (SHA-256). Non supporté dans Packet Tracer.
...
**Simulation vs équipements physiques.** Packet Tracer reproduit fidèlement le comportement Cisco mais ne couvre pas toutes les subtilités des équipements physiques.

---

## 8. Recommandations pour une PME

*[À rédiger par Dimitri — formule 5 à 7 recommandations concrètes et actionnables pour une PME comme NordLogistique. Chaque recommandation doit nommer le risque adressé et la solution proposée. Appuie-toi sur ce que tu as construit et sur les limites identifiées en section 7.]*

---

## 9. Conclusion

*[À rédiger par Dimitri — synthèse de ce que ce projet démontre, ce que tu as appris, et en quoi cette infrastructure constitue une base solide et défendable en entretien. Environ 150 mots.]*
